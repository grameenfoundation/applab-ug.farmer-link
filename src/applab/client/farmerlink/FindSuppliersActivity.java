...
public class FindSuppliersActivity extends ListActivity {
    ...
    static final int ADD_SUPPLIER_DIALOG = 1;
    ...
    public void buttonClicked(View view) {
        switch (view.getId()) {
            case R.id.next_add_farmers:
                Intent nextIntent = new Intent(getApplicationContext(), TransportEstimatorBuyingActivity.class);
                nextIntent.putExtra("source", "Buyer: ");
                MarketSaleObject.getMarketObject().setFarmers(suppliers);
                startActivity(nextIntent);
                break;
            case R.id.back_find_farmers:
                Intent backIntent = new Intent(getApplicationContext(), AddBuyerActivity.class);
            ...
            if (suppliers.size() > 0) {
                if (suppliers.get(position).getName().equalsIgnoreCase("NONE")) {
                    ...
                    if (crop.equalsIgnoreCase(suppliers.get(position).getCropOne())) {
                        amountView.setText("Amount : " + String.valueOf(suppliers.get(position).getAmountCropOne()) + " Kgs");
                    }
                    else if (crop.equalsIgnoreCase(suppliers.get(position).getCropTwo())) {
                        amountView.setText("Amount : " + String.valueOf(suppliers.get(position).getAmountCropTwo()) + " Kgs");
                    }
                    else if (crop.equalsIgnoreCase(suppliers.get(position).getCropThree())) {
                        amountView.setText("Amount : " + String.valueOf(suppliers.get(position).getAmountCropThree()) + " Kgs");
                    }
                }
            }
            else {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        selectedSupplierIndex = (int)info.id;
        Farmer supplier = suppliers.get(selectedSupplierIndex);
    ...
    private void selectSupplier(long id) {
        showDialog(ADD_SUPPLIER_DIALOG);
        setListAdapter(new SuppliersAdapter());        
    }
    ...
    protected Dialog onCreateDialog(int dialogId) {
        switch (dialogId) {
            ...
            case ADD_SUPPLIER_DIALOG:
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
                // Get the layout inflater
                LayoutInflater inflater = getLayoutInflater();
                View dialogView = inflater.inflate(R.layout.supplier_dialog, null);
                final EditText dialogAmountView = (EditText)dialogView.findViewById(R.id.dialog_amount);
                final EditText dialogPriceView = (EditText)dialogView.findViewById(R.id.dialog_price);
                ...
                // Add action buttons
                       .setPositiveButton("Add", new DialogInterface.OnClickListener() {
                           ...
                           public void onClick(DialogInterface dialog, int id) {
                               Farmer currentFarmer = suppliers.get(selectedSupplierIndex);
                                                             
                               String amount = dialogAmountView.getText().toString();
                               String price = dialogPriceView.getText().toString();
                               
                               // TODO: remove the hardcoded bits
                               currentFarmer.setQuantity(Double.valueOf(amount));
                               currentFarmer.setPreferredPrice(Double.valueOf(price));
                               currentFarmer.setSelected(true);
                               setListAdapter(new SuppliersAdapter());
                           }
                       })
